...
rio::import("Observational_data.csv")
rio::import("MendelianRandomization_data.csv")
rio::import("ProteinInstruments_annotation.csv")
rio::import("ProteinsMeasurements_annotation.csv")
rio::import("Metabolites_annotation.csv")
...
ui <- fluidPage(page_navbar(
  ...
  nav_panel(title = "Welcome", 
            ...
            p("In order to see the results, please click the ", tags$i("Observational analyses"),  " tab in the ", tags$i("Tables"), " tab at the top of this page and then enter the name of a protein or a metabolite to see the results. Two degrees of adjustment was used, age and sex-adjustment and also additional adjustment for BMI and kidney function (eGFR)."),
            ...
            p("The results could also be downloaded by pressing the ", tags$i("Download"), " button."),
  ...
  nav_panel(title = "Tables", 
            tabsetPanel(
              ...
              tabPanel(title = "Observational analyses",
                       ...
                       # tags$style(".nav-link-color {color:DarkSlateGrey;}"),
              ...
              tabPanel(title = "Mendelian Randomization",
                       ...
                       # tags$head(tags$style(HTML("#metabolite_obs + div > .selectize-input {height: 20px; width: 550px; font-size: 16px;}"))),
                       # tags$head(tags$style(HTML("#protein_obs + div > .selectize-input {height: 20px; width: 550px; font-size: 16px;}"))),
  ...
  nav_panel(title = "Heatmaps", 
            tabsetPanel(
              ...
              tabPanel(title = "Observational analyses",
                       ...
                       p(""),
                       p(""),
                       tags$span(style = "color:black; font-size:13pt", "Please select a super- or sub-pathway, and/or a protein and click", tags$i("Plot!."), "If you select the boolean operator OR, you will see all the associations for that pathway or protein.") ,
                       p(""),
                       p(""),
                       p(""),
                       ...
                       fluidPage(
                         ...
                         tags$div(  selectizeInput("protein_obs_plot", "Select or type a protein", choices = NULL, multiple = FALSE,
                                                   selected = NULL,
                                                   options = list('plugins' = list('remove_button'),
                                                                  placeholder = '' ) )          ,  style="display:inline-block"),
                         tags$div( tags$div("") , style="display:inline-block; width: 70px;"  ),
              ...
              tabPanel(title = "Mendelian Randomization",
                       ...
                       fluidPage(
                         ...
                         tags$div(  selectizeInput("protein_mr_plot", "Select or type a protein", choices = NULL, multiple = FALSE,
                                                   selected = NULL,
                                                   options = list('plugins' = list('remove_button'),
                                                                  placeholder = '' ) )          ,  style="display:inline-block"),
                         tags$div( tags$div("") , style="display:inline-block; width: 70px;"  ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$button_obs_plot,{
    ...
    if ( input$pathway_obs_plot == "" && input$protein_obs_plot == "" ) {
      showFeedbackWarning(
    ...
    } else if( input$protein_obs_plot == "" && input$boolean_obs_plot == "AND" ){
      showFeedbackWarning(
        inputId = "protein_obs_plot",
        text = "Please select a protein from the list"
      )
  ...
  ht_obs <- observeEvent(input$button_obs_plot,{
    ...
    stars_pvalues_cell <- function(j, i, x, y, w, h, fill) {
      ...
      if(pvalues[i, j] < 0.01 ) {
        ...
        grid.text(" ", x, y - gb_h*0.35, gp = gpar(fontsize = 16))
      } else if (pvalues[i, j] == 1.5) {
        grid.segments(x0 = x+w*0.47, y0 = y+h*0.47,
                      x1 = x-w*0.47, y1 = y-h*0.47,
                      gp=gpar(col = "#2C2C2C",
                              lineend = "round",
                              lwd = unit(1, "pt")) )
      }
    ...
    if( nrow( df_filtered ) > 200 ){
      ...
      ht <- Heatmap(  betas, name = "Beta", col = col_fun,
                      ...
                      # column_title = paste0(input$pathway_mr_plot, " Pathway"),
                      ...
                      # column_title_gp = gpar(fontsize = 11.5),
                      column_names_rot = 45,
                      ...
                      heatmap_legend_param = list(
                        ...
                        # legend_height = unit(dim(betas)[1]*0.2, "cm"),
  ...
  observeEvent(input$button_mr_plot,{
    ...
    if ( input$pathway_mr_plot == "" && input$protein_mr_plot == "" ) {
      showFeedbackWarning(
    ...
    } else if( input$protein_mr_plot == "" && input$boolean_mr_plot == "AND" ){
      showFeedbackWarning(
        inputId = "protein_mr_plot",
        text = "Please select a protein from the list"
      )
